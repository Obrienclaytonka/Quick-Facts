...
The table below provides a summary of the 2023 Hypoxia Season at a glance. Data from all CTDEEP cruises can be downloaded from the [UCONN ERDDAP](http://merlin.dms.uconn.edu:8080/erddap/tabledap/DEEP_WQ.html). All samples are collected and analyzed under EPA-approved Quality Assurance Project Plans.
...
*Click on the Tabs to explore more details.*  
...
Avg_plot <- ggplot (data=MovAvg)+
  ...
  scale_y_continuous (
    name= "Area", 
    sec.axis = sec_axis(trans=~.*0.05, name="Second Axis"))
...
Vol_plot <- ggplot (data=HypoxicVol)+
  geom_line(mapping = aes(x=Year, y=Volume)) +
              labs(x= "Year", y= "Volume (km^3)", title= "Volume of Long Island Sound Bottom Waters with Dissolved Oxygen Concentrations Below 3.0 mg/L")
...
dt <- read_excel("C:\\R\\Data\\2023_hypoxAreaVol.xlsx")

dt %>%
  kbl(caption =  "Volume Estimates of Hypoxic Long Island Sound Bottom Waters Obtained from the UCONN HypVolApp", align = "c", col.names = c("CRUISE", "2 mg/L", "3 mg/L", "4.8 mg/L", "2 mg/L", "3 mg/L", "4.8 mg/L")) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
...
### **Frequency of Hypoxia (1994-2023)**
...
percent_plot <- ggplot(data=freq_94_23) + 
  geom_col(aes(x = Station_Name, y = Percentage, width=0.8, fill=Percentage, label1=Percentage, label2=Station_Name)) +
      labs(title = "Percentage of Years Station Has Been Hypoxic Between 1994 and 2023", 
        y= "Percentage", 
        x= "Station") +
...
ggplotly(percent_plot, tooltip = c("label1", "label2"), fig.align='center')
